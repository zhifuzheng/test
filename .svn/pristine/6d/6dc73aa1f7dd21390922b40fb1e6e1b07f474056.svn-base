	...
	<form id="itemAddForm" class="itemForm" method="post">
		<table style="border-collapse:separate; border-spacing:30px 30px;">
			...
			<tr>
				<td>会员价格:</td>
				<td>
					<input class="easyui-numberbox" type="text" name="vipPriceView" data-options="min:0,max:99999999,precision:2,required:true" style="width:280px;"/>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$.ajax({
			...
			data:{'entityTableName':'business_apply','approvalStatus':1,'shopState':1,'applyType':0},
			type:"post",
			success:function(data){
				if(data.length == 0){
					$.messager.alert('提示','还没有任何一个零售商加盟哦','warning');
				}else{
					var loadData = [];
	...
	function submitForm(){
		...
		$("#itemAddForm [name=vipPrice]").val($("#itemAddForm [name=vipPriceView]").val() * 100);
		//佣金比例
		...
		$.post(
			...
			function(data){
				if(data.status == "1"){
					...
					loadCenterLayout('shop/platItemAdd.jsp');
				}else{
