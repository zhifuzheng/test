...
<style>
	...
	*::-webkit-scrollbar-track{
	...
	<div style="width:99%;height:50px;" id="condition">
		<div class="left" style="line-height: 50px;margin-left: 10px;width:15%;">
			<span>排序：</span>
			<select class="select-box" id="range" onchange="getCouponListby(1)">
		...
		<div class="left" style="line-height: 50px;margin-left: 10px;width:20%;">
			<span>发行方式：</span>
			<select style="width: 70%;" id="releaseType" onchange="getCouponListby(5)">
				<option value="noLimit">不限</option>
		...
		<div class="left" style="line-height: 50px;margin-left: 10px;width:15%;">
			<span>类型：</span>
			<select id="type" onchange="getCouponListby(2)">
				<option value="noLimit">不限</option>
		...
		<div class="left" style="line-height: 50px;margin-left: 10px;width:15%;">
			<span>状态：</span>
			<select id="status" onchange="getCouponListby(3)">
				<option value="noLimit">不限</option>
		...
		<div style="line-height: 50px;width:30%;float: right;">
			...
			<input onclick="getCouponListby(4)" type="submit" style="cursor: pointer;color:#ffffff;width:13%;line-height:30px;background-color: rgba(0, 0, 255, 0.7);border-radius: 5px;margin-left:2%" value="搜索">
			<!-- <button onclick="getCouponListby(4)" style="color:#ffffff;width:13%;line-height:30px;background-color: rgba(0, 0, 255, 0.7);border-radius: 5px;margin-left:2%">搜索</button> -->
		</div>
	</div>
	...
	<div id="create-down-QRcode" style="display: none;">
		<form id="integralRule" method="post">
			<table style="width: 50%; margin: auto;border-bottom:0px solid #c5c5c5;">
				...
				<tr style="display: block; margin: 10px; width: 100%">
					...
					<td style="width: 250px;">
						<input id="QRcode-number" type="number" class="easyui-textbox" name="QRcodeNumber" style="width: 100%" />
					</td>
				</tr>
			</table>
		</form>
		...
		<div style="width: 50%; margin-top: 10px; margin-left: 220px; color: #8e8e8e;">
...
$(function(){
	...
	$("#data-list").datagrid({
		url:'<%=path %>'+"/system/coupon/platformFindCouponList",
		queryParams:{
			range:range,
			type:type,
			status:status,
...
function getCouponListby(e){
	...
	var searchWords = $("#searchWords").val();
	var releaseType = $("#releaseType").val();
	$("#data-list").datagrid('reload',{'range':range,'type':type,'status':status,'searchWords':searchWords,'releaseType':releaseType});
}
...
function editorCoupon(batchUUID){
	/* 编辑未上架的优惠劵 */
	loadCenterLayout("coupon/platformEditorCoupon.jsp?batchUUID="+batchUUID);
}

/* 优惠劵使用详情 */
function getCouponUseRecord(batchUUID){
	loadCenterLayout("coupon/platformCouponUseDetail.jsp?batchUUID="+batchUUID);
}
	
	var index = null;
	var batchUUID = null;
	/* 线下优惠劵二维码下载 */
	function createAndDownQRcode(){
	...
	function downloadCoupon(e,f){
		 if(e == 1 || e == 2){//下载
			...
			//console.log("batchUUID:"+batchUUID);
			/* 发送请求获取二维码图片 */
			$.ajax({
