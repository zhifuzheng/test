...
<div style="padding:10px 10px 10px 10px">
	<form id="couponAdd" class="couponForm" method="post">
		<table style="width:100%;">
						...
						<table style="width: 80%">
							<thead>
								<tr style="background-color: #f4f4f4;height:30px;">
									<th class="th">店铺名称</th>
									<th class="th">店铺地址</th>
									<th class="th" style="border-right:1px solid #c5c5c5;text-align:center;">操作</th>
								</tr>
							</thead>
			...
			<tr class="breadth" style="height:40px;width:100%;">
				<td class="breadthT" style="width:8%">是否叠加使用:</td>
				<td>
		...
		<div style="padding:5px;width: 866px; text-align: right;margin-bottom:100px;">
...
<script type="text/javascript">
	...
	function submitForm(){
		...
		$.ajax({
			...
			data:couponAdd+"&merchantUUID="+businessUUID+"&basePath="+url+"&fromType=1",
			type:"POST",
			dataType:"json",
			success:function(data){
				...
				if(data.status == "1"){
					...
					loadCenterLayout('coupon/platformCreateCoupon.jsp');
				}else{
			...
			success:function(data){
				if(data.status == "1"){
					for(var i = 0;i<data.businessList.length;i++){
						...
						'<td class="td" style="width: 10%;border-right:1px solid #c5c5c5;text-align:center;"><input type="checkbox" name="businessUUID" value="'+data.businessList[i].businessUUID+'"></td>'+
						'</tr>';
					...
					if(data.status == "1"){
						for(var i = 0;i<data.sortList.length;i++){
							var html = '<tr style="border-bottom: 1px solid #f4f4f4;height:30px">'+
								...
								'<td style="width: 30%;border-right:1px solid #c5c5c5;text-align:center;" class="td"><input type="radio" onclick="getList(this)" name="catUUID" value="'+data.sortList[i].catUUID+'"></td>'+
								'</tr>';
	...
	$(document).ready(function() {
		...
		$("#goods-box").hide();
		$("#sort-box").hide();
		//默认一开始不限制店铺，所以隐藏分类和商品，只有限制店铺时才显示
		$("#sort-tr").css("visibility","collapse");
		$("#goods-tr").css("visibility","collapse");
	})
