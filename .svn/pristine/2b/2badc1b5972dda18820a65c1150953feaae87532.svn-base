...
function loadMenuTree(){
	var url="<%=path%>/Public/gx/findMenuTree"; 
...
function zTreeBeforeRemove(treeId, treeNode) {	
	selectNode = treeNode;
	var url="<%=path%>/Public/gx/delMenu?menuUUID="+treeNode.menuUUID;
...
function addOrUpMenu(menuUUID,menuParentCode,menuParentName,type){//新增/修改菜单	
	...
	var url = "gxmenu/gx_menu_save.jsp?menuUUID="+menuUUID+"&menuParentCode="+menuParentCode+"&menuParentName="+menuParentName+"&isEdit="+type;
	var title = '新增菜单';
	if("1"==type){
		title = '修改菜单';
	}
