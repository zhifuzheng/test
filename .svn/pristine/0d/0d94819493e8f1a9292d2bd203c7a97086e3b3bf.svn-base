...
$(function(){
	   ...
	   if("1"=="<%=isEdit%>" && ""!="<%=menuUUID%>"){
		   //是编辑数据
		   var url="<%=path%>/Public/gx/findMenu?menuUUID=<%=menuUUID%>";
		   $("#addMenu_form").form('load', url);
			/* $.post(url,{},function(data){
				$("#menuId").val(data.id);
				$("#menuUUID").val(data.menuUUID);
				$("#menuName").textbox("setValue",data.menuName);
				$("#menuMark").textbox("setValue",data.menuMark);
			},'json'); */
	   }else{
		   //是添加数据
...
function saveMenu(){
	   $.post("<%=path%>/Public/gx/menuSave",$("#addMenu_form").serialize(),function(data){
		  if(data){
