...
function _mediaImg(blankPath,attrs,zdy) {
	...
	if (attrs.src.indexOf(".mp4") == -1 && attrs.src.indexOf(".MP4")) {
	     alert("只支持mp4格式的视频，插入其他格式的视频将无法正常播放");
	     return;
    }
	var html = '<video poster="" controls="" width="100%" height="auto" class="' + _mediaClass(type) + '" ';
...
_extend(KUploadButton, {
	init : function(options) {
		...
		K('.ke-upload-area', div).width(width+5);
		self.options = options;
	},
	submit : function() {
		var self = this,
