...
%>
	...
	<div class="easyui-layout" data-options="fit:true">
		<div data-options="region:'west'" style="width:50%;">
			...
			<div id="item_tb" style="padding:2px 5px;">
				<input id="entityName" class="easyui-combobox" data-options="required:true,editable:false,'prompt':'没有查询到零售商店铺哦'" style="width:200px;" />		
	...
	<script type="text/javascript">
		...
		function itemEdit(rowIndex){
			...
			$("<div>").css({padding:"5px"}).dialog({
			    ...
			    onOpen : function(){
			    	...
			    	template.find("[comboname=distributeStatus]").combobox("select",row.distributeStatus);
			    ...
			    buttons:[{
					...
					handler:function(){
						...
						var totalRatio = eval(template.find("[name=firstRatioView]").val())+eval(template.find("[name=secondRatioView]").val());
						...
						//佣金比例
						template.find("[name=firstRatio]").val(eval(template.find("[name=firstRatioView]").val()) / 100);
						template.find("[name=secondRatio]").val(eval(template.find("[name=secondRatioView]").val()) / 100);
						...
						$.post(
							...
							function(data){
								if(data.status == "1"){
									...
									row.distributeStatus = template.find("[name=distributeStatus]").val();
									if(row.distributeStatus == 1){
										row.firstRatio = template.find("[name=firstRatio]").val();
										row.secondRatio = template.find("[name=secondRatio]").val();
									}
			...
			var distributeData = [{'text':'关闭','value':0},{'text':'开启','value':1}];
			distributeData[row.distributeStatus].selected = true;
			$("#distribute").combobox({
			    data : distributeData,
			    valueField : 'value',
			    textField : 'text',
			    onChange : function(){
	                ...
	                if(value == 1){
				        ...
				        distribute.find("[name=firstRatio]").val(row.firstRatio);
				        distribute.find("[name=secondRatio]").val(row.secondRatio);
				        distribute.find("[numberboxname=firstRatioView]").numberbox("setValue",row.firstRatio*100);
				        distribute.find("[numberboxname=secondRatioView]").numberbox("setValue",row.secondRatio*100);
		...
		function itemLook(rowIndex){
			...
			$("<div>").css({padding:"5px"}).window({
			    ...
			    onOpen : function(){
			    	...
			    	html += '<tr>';
			    	html += '<td><span style="color:red;">分销状态</span>:' + (row.distributeStatus == "0"?"关闭":"开启") + '</td>';
			    	html += '<td><span style="color:red;">配送方式</span>:门店自取</td>';
			    	html += '<td><span style="color:red;">商品原价</span>:' + (row.price/100).toFixed(2) + '</td>';
			    	html += '<td><span style="color:red;">商品现价</span>:' + (row.tagPrice/100).toFixed(2) + '</td>';
			    	html += '</tr>';
		...
		function queryItem(){
			...
			$('#item_table').datagrid('reload',{"itemStatus":itemStatus,'entityUUID':entityUUID,'isDel':isDel,"title":condition});
			$('#param_table').datagrid('loadData',{total:0,rows:[]});
		}
		...
		function paramAdd(){
			...
			var template = $('<div>\
				<form class="itemForm" method="post">\
					...
					<input type="hidden" name="firstRatio"></input>\
					<input type="hidden" name="secondRatio"></input>\
				    <table style="border-collapse:separate; border-spacing:50px 30px;">\
				        ...
				        <tr>\
			            	<td>会员价格:</td>\
				            <td>\
				            	<input class="easyui-numberbox" type="text" name="vipPriceView" data-options="min:1,max:99999999,precision:2,required:true" style="width:280px;"/>\
			...
			$("<div>").css({padding:"5px"}).dialog({
			    ...
			    onOpen : function(){
			    	...
			    	template.find("[name=firstRatio]").val(row.firstRatio);
			    	template.find("[name=secondRatio]").val(row.secondRatio);
			    ...
			    buttons:[{
					...
					handler:function(){
						...
						//取商品价格，单位为“分”
						template.find("[name=vipPrice]").val(eval(template.find("[name=vipPriceView]").val()) * 100);
						
						//取商品的等级价格
						var gradeJson = {};
						gradeJson['0'] = template.find("[name=priceView]").val() * 100;
						template.find("[name='gradeData']").val(JSON.stringify(gradeJson));
						...
						var price = template.find("[name=priceView]").val() * 100;
						var gradeData = {'0' : price};
						template.find("[name='gradeData']").val(JSON.stringify(gradeData));
						//template.find("form").serialize()将表单序列号为key-value形式的字符串
						$.post(
		...
		function paramEdit(rowIndex){
			...
			if(row.paramData != null){
				...
				for(var key in paramData){
					...
					var paramsTr = $('<tr><td style="width:92px;height:60px;">'+key+': </td><td></td></tr>');
			...
			$("<div>").css({padding:"5px"}).dialog({
			    ...
			    onOpen : function(){
			    	...
			    	template.find("[textboxname=title]").textbox("setValue",row.title);
			    	template.find("[numberboxname=priceView]").numberbox("setValue",(row.price/100).toFixed(2));
			    	template.find("[numberboxname=vipPriceView]").numberbox("setValue",(row.vipPrice/100).toFixed(2));
						...
						$.post(
							...
							function(data){
								if(data.status == "1"){
									...
									row.title = template.find("[name=title]").val();
									row.price = template.find("[name=price]").val();
									row.vipPrice = template.find("[name=vipPrice]").val();
