...
<body>
	...
	<div class="hidden">
		<script type="text/javascript">
			$(function (){
				setMultiSelectVal("<%=unitUUID%>","<%=vipUUID%>");
			});
			
		//设置multiselect的值
		function setMultiSelectVal(unitUUID,vipUUID){
		    //查所有菜单信息
	    	var url = "<%=path%>/system/roleVip/findRoleToAssignRole?unitUUID="+unitUUID+"&vipUUID="+vipUUID;
	    	var htmlOption = "";
	    	$.post(url,null,function(data){
				if(data!="[]"){
					for(var i=0;i<data.length;i++){
						htmlOption += "<option value='"+data[i].roleUUID+"' "+ data[i].selected +">"+data[i].roleName+"</option>";
					}
				}
		...
		function getFromSerialize() {
			return $('#vipRoleForm').serialize();
		}
		//修改数据时
		function initData() {
			$('#vipUUID').val("<%=vipUUID%>");
			$('#unitUUID').val("<%=unitUUID%>");
			$('#roleUUIDS').val($("#selectMenuUser").val());
		}
		</script>
	</div>
</body>
