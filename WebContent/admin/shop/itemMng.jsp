	...
	<div class="easyui-layout" data-options="fit:true">
		<div data-options="region:'west'" style="width:50%;">
			...
			<div id="item_tb" style="padding:2px 5px;">
				<input class="easyui-combobox" name="entityName" id="entityName" data-options="prompt:'请输入店铺名称'" style="width:200px;" />
				...
				<input id="condition" data-options="'prompt':'请输入商品名称/现价'" class="easyui-textbox" style="width:200px;">				
	...
	<script type="text/javascript">
		...
		var columns = [ [ {
			...
			formatter : function(data,row,index){
				...
				if(row.itemStatus == "1"){
					...
					html += '&nbsp;&nbsp;<a class="easyui-linkbutton button-warning button-xs l-btn l-btn-small" data-options="plain: true, iconCls: \'icon-remove\'" onclick="changeStatus({\'index\':\''+index+'\',\'status\':\'3\'});">&nbsp;违规&nbsp;</a>';
				...
				if(row.itemStatus == "3"){
					html = '<a class="easyui-linkbutton button-success button-xs l-btn l-btn-small" data-options="plain: true, iconCls: \'icon-remove\'" onclick="changeStatus({\'index\':\''+index+'\',\'status\':\'1\'});">&nbsp;恢复&nbsp;</a>';
		...
		$(function(){
			$.ajax({
				...
				data:{'entityTableName':'business_apply','applyType':itemType},
				type:"post",
			...
			$.messager.confirm("操作提醒", "确定要删除商品\""+row.title+"\"吗？"+msg, function (c) {
				if(c){
					$.ajax({
						...
						success:function(data){
							if(data.status == "1"){
								$('#item_table').datagrid('reload',{'itemType':itemType,'isDel':isDel});
		...
		function changeStatus(content){
			...
			if(itemStatus == "3"){
				msg = "确定要将商品\"" + row.title + "\"标记为违规商品，并强制下架吗？";
			}
					...
					$.ajax({
						...
						success:function(data){
							if(data.status == "1"){
								...
								if(isDel == "1"){
									$('#item_table').datagrid('reload',{'itemType':itemType});
								}
		...
		function itemEdit(rowIndex){
    		...
    		},'catUUID':row.catUUID,'catCode':row.catCode,'catName':row.catName,'rootUUID':row.parentUUID,'rootName':''});
		...
		function queryItemStatus(data){
			...
			$('#item_table').datagrid('reload',{'itemType':itemType,'itemStatus':itemStatus,'isDel':isDel});
			$('#param_table').datagrid('loadData',{total:0,rows:[]});
		}
		...
		function queryItem(){
			...
			if(regPos.test(condition) && condition.length != 11){
				...
				$('#item_table').datagrid('reload',{'itemType':itemType,'itemStatus':itemStatus,'isDel':isDel,'entityName':entityName,"price":condition});
				return;
			}
			$('#item_table').datagrid('reload',{'itemType':itemType,'itemStatus':itemStatus,'isDel':isDel,'entityName':entityName,'title':condition});
			$('#param_table').datagrid('loadData',{total:0,rows:[]});
		}
			...
			var template = $('<div>\
				<form class="itemForm" method="post">\
				    ...
				    <table style="border-collapse:separate; border-spacing:50px 30px;">\
			            ...
			            <td>套餐价格:</td>\
				            <td>\
				            	<input class="easyui-numberbox" type="text" name="priceView" data-options="min:1,max:99999999,precision:2,required:true" style="width:280px;"/>\
		...
		function initParamQuery(){
			...
			$('#param_table').datagrid('reload',{"paramStatus":paramStatus,"itemUUID":itemUUID});
		}
