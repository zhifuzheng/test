...
public class CodeUtil {
	 ...
	 * @return
	 */
	public static String[] getCode2Arr(){
		return strArr2;
	}
	/**
	 * 获取2位长度编码
	...
	public static String getCode2(int n) {
		if(n>strArr2.length-1){
			return "";
		}
		return strArr2[n];
	}
	/**
	 * 获取3位长度数组，全系统第一次请求时创建
	 * @return
	 */
	public static String[] getCode3Arr(){
		if(strArr3!=null){
			return strArr3;
		}else{
			strArr3 = new String[46656];
			int n = 0;
			for (int i = 0; i < strArr.length; i++) {
				for (int k = 0; k < strArr.length; k++) {
					for (int j = 0; j < strArr.length; j++) {
						strArr3[n]=strArr[i]+strArr[k]+strArr[j];
						++n;
					}
				}
			}
	...
	/**
	 * 获取3位长度编码，全系统第一次请求时创建
	 * @param m
	 * @return
	 */
	public static String getCode3(int m){
		if(strArr3!=null){
			if(m>strArr3.length-1){
				return "";
			}
			return strArr3[m];
		}else{
	...
	public static void testFile() throws IOException{
		Connection con = new Connection("192.168.8.2", 22);
		//连接
		...
		boolean isAuthed = con.authenticateWithPassword("root","rootaz");
		...
		scpClient.get("/home/x.txt", "C:/y.txt");
		//将本地文件上传到服务器端的目录下
		scpClient.put("C:/RUNNING.txt", "/home/oracle");
	...
	public static void main(String[] args) {
		...
		float xx = Float.valueOf("50")/Float.valueOf("100");
		/*
		int n = 0;
		for (int i = 0; i < strArr.length; i++) {
			for (int k = 0; k < strArr.length; k++) {
				for (int j = 0; j < strArr.length; j++) {
					 sbu.append("\"").append(strArr[i]).append(strArr[k]).append(strArr[j]).append("\",");
					++n;
				}
			}
		}
